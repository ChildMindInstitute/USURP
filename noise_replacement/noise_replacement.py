...
def timeshift_adults(path, borders, rate):
    """
    Function to replace all marked adult segments.
    ...
    Returns
    -------
    timeshifted : pydub audio segment
        the sound with the adults timeshifted out
    ...
    """
    out_path = os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(
               path))), "adults_timeshifted", os.path.basename(path))
    if not os.path.exists(os.path.dirname(out_path)):
        os.makedirs(os.path.dirname(out_path))
    original = pydub.AudioSegment.from_wav(path)
    timeshifted = build_new_soundfile(original, rate, None, borders)
    print(''.join(["Saving ", out_path]))
    timeshifted.export(out_path, format="wav")
    return(timeshifted)
            ...
            if os.path.isdir(ar_path):
                for wav_file in os.listdir(ar_path):
                    if wav_file.endswith('.wav'):
                        ...
                        timeshift_adults(original_path, silences, 44.1)
